...
async def play_music(_, message: Message, context: dict):
    ...
    if url:
        ...
        try:
            ...
            await hell.edit("YouTube failed. Trying JioSaavn...")
            result = await get_jiosaavn_data(url)
            if not result:
    ...
    try:
        ...
        # If YouTube fails, try JioSaavn
        await hell.edit("YouTube failed. Trying JioSaavn...")
        result = await get_jiosaavn_data(query)
        if not result:
            return await hell.edit(f"**Error:**\n`{e}`")
    context = {
        "chat_id": message.chat.id,
...
async def get_jiosaavn_data(query):
    api = JioSaavnAPI()
    search_result = api.search_for_song(query)
    ...
    result = [{
        "id": song['id'],
        "title": song['song'],
        "duration": formatter.secs_to_mins(int(song['duration'])),
        "url": song['media_url'],
